...
class SamplePlugin(PluginBase):
    ...
    def on_load(self) -> bool:
        """Проверка состояния плагина при загрузке"""
        ...
        if not output_dir:
            ...
            return True  # Плагин загружен, но работать пока не может
        ...
        self.main_window.log_message("info", "Плагин Cleanup Plugin готов к запуску")
        return True

    def run(self):
        """Метод для выполнения действия плагина при нажатии кнопки 'Запуск'"""
        output_dir = self.main_window.config.get("output_dir")
        if not output_dir:
            self.main_window.log_message("warning", "Плагин: Папка для сохранения не выбрана.")
            return
